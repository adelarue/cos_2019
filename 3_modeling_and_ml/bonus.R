...
commentsTM_bonus <-
  commentsTM_bonus[, !grepl("[0-9]", names(commentsTM_bonus))]
...
# Fit the forest with 200 trees, using the first
# (400 - 2) words alphabetically
...
# Check accuracy
t <- table(commentsTest_bonus$PERFECT_RATING, Airbnb.forestPred)
sum(diag(t)) / nrow(commentsTest_bonus)
...
# Make predictions on the test set
Airbnb.SVMPred <- predict(Airbnb.SVM_cv$best.model,
                          newdata = commentsTest_bonus)
table(commentsTest_bonus$PERFECT_RATING, Airbnb.SVMPred)

# Check accuracy
t <- table(commentsTest_bonus$PERFECT_RATING, Airbnb.SVMPred)
sum(diag(t)) / nrow(commentsTest_bonus)

# Also, for nice visualizations of SVM, including the support vectors,
# see Jerry's code from the 2016 course:
